...
function createChecksum(array){
	...
	//console.log("arr", arr);
		...
		arr.forEach( function(letter, ii, arr) {

			let result = getAllIndexes(arr, letter);
	//console.log("arr, letter, ii, result", arr, letter, ii, result)
			if(result.length == 2 && !doubleFound){
				doubles += 1;
				doubleFound = true;
			}
			else if(result.length == 3 && !tripleFound){
				triples += 1;
				tripleFound = true;
			}
	//console.log("doubles, triples", doubles, triples)
		})
	})

	return  "doubles * triples = " + (doubles * triples)
}
